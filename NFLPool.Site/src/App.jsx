...
function App() {
    ...
    const nflAPIClient = axios.create({
        baseURL: "https://localhost:7259/api/Score"
    });
    ...
    const refreshPool = () => {
        fetchWeekResults(year, week);
    };

    const fetchWeekResults = async (year, week) => {
        setGameScores([]);
        setParticipants([]);
        setShowPool(false);
        let response = await nflAPIClient.get('?year=' + year + '&week=' + week + '&seasontype=2');
    ...
    return (
        <>
            ...
            {!showPool && <div className="loading"><img src={spinningFootball} alt="loading..." /></div>}
            {showPool &&
